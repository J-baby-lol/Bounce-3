 ...
 * the ShapeModelListener interface so that their instances can be registered as
 * listeners on a ShapeModel. Whenever a Shape is added to or removed from a
...
public class ShapeModel {
	...
	public ShapeModel(Dimension bounds) {
		fRoot = new NestingShape(0, 0, 0, 0, bounds.width, bounds.height);
		fBounds = bounds;
	 ...
	 * already part of a NestingShape. If the shape can be added, it is and this
	 * method fires a ShapeModelEvent to registered listeners alerting them of
	 * the new shape.
	 * 
	 * @param shape
	 *            the new shape to add to this ShapeModel.
	 * @param parent
	 *            the intended parent of the new shape.
	 */
	...
	public void remove(Shape shape) {
		...
		if (parent != null) {
			...
			// Fire event.
			fire(ShapeModelEvent.makeShapeRemovedEvent(shape, parent, index,
					this));
		}
	}
	 ...
	 * ShapeModel to move before notifying each registered ShapeModelListener of
	 * the movement. Note that a clock() call results in ONE ShapeModelEvent
	 * being fired; the event identifies the root NestingShape.
	 */
	public void clock() {
		fRoot.move(fBounds.width, fBounds.height);

		// Fire event.
		fire(ShapeModelEvent.makeShapeMovedEvent(fRoot, this));
	}
